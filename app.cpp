...
int getValue(rgb_raw_t rgb); /* 明度を求める */
...
void main_task(intptr_t unused)
{
   ...
   */
  ev3_sta_cyc(BALANCER_CYC);
  slp_tsk();
  ev3_stp_cyc(BALANCER_CYC);
...
/* バランサー周期ハンドラ */
void balancer_cyc(intptr_t unused)
{
    act_tsk(BALANCER_TASK);
}

/* バランサータスク */
void balancer_task(intptr_t unused)
{
    rgb_raw_t rgb;
    int value = 0;
    ...
    else
    {
        ...
        value = getValue(rgb);
        if (value >= (LIGHT_WHITE + LIGHT_BLACK)/2)
        {
            turn =  20; /* 右旋回命令 */
        }
        else
    ...
    fprintf(bt, "%u,%u,%u,%d,%lu\n", rgb.r, rgb.g, rgb.b, value, systim - starttim);
...
/* 明度を求める */
int getValue(rgb_raw_t rgb)
{
    int value;

    value = (rgb.r > rgb.g) ? rgb.r : rgb.g;
    if (rgb.b > value) value = rgb.b;
